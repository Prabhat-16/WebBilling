...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertItem` (IN `INAME` VARCHAR(30), IN `QID` BIGINT(20), IN `CID` BIGINT(20), IN `SID` VARCHAR(100), IN `GID` VARCHAR(100), IN `QTY` BIGINT(50), IN `AMT` BIGINT(100), IN `OPSTOCK` BIGINT(50), IN `ISACTIVE` TINYINT(1), IN `ITEMPHOTO` VARCHAR(2000), IN `UID` BIGINT(20))   BEGIN
	INSERT INTO tbl_item (item_name,quality_id,category_id,size_id,gsm_id,quantity,amount,op_stock,is_active,item_photo,created_by) VALUE (INAME,QID,CID,SID,GID,QTY,AMT,OPSTOCK,ISACTIVE,ITEMPHOTO,UID);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateItem` (IN `IID` BIGINT(20), IN `INAME` VARCHAR(30), IN `QID` BIGINT(20), IN `CID` BIGINT(20), IN `SID` VARCHAR(100), IN `GID` VARCHAR(100), IN `QTY` BIGINT(50), IN `AMT` BIGINT(100), IN `OPSTOCK` BIGINT(50), IN `PTY` BIGINT(50), IN `CTP` DATE, IN `ISACTIVE` TINYINT(1), IN `ITEMPHOTO` VARCHAR(2000), IN `UID` BIGINT(20))   BEGIN
	UPDATE tbl_item SET item_name  = INAME , quality_id = QID , category_id = CID , size_id = SID , gsm_id = GID , quantity = QTY , amount = AMT , op_stock = OPSTOCK , penalty = PTY , credit_period = CTP ,  is_active = ISACTIVE , item_photo = ITEMPHOTO , created_by = UID WHERE item_id = IID;
END$$
...
(13, 'Prabhat Mishra', 7874063236, 1, '2024-03-01', 'Hello Last King', 12093.05, 10000.00, 1, 4, 2, 4),
(14, 'Deep Pandya', 6325874123, 2, '2024-03-02', 'Taken Goods For Papers Added Silver', 36535.50, 2000.00, 5, 1, 2, 4),
(15, 'Prashant Mishra', 2365412301, 3, '2024-03-02', 'Testing For Goods Purpose ', 30848.92, 3000.00, 4, 4, 2, 4);
...
(31, 15, 1, 110.00, 20.00, 2.00, 44.00, 1.00, 21.56, 2.00),
(24, 14, 21, 10.00, 20.00, 1.00, 2.00, 0.00, 0.00, 2.00),
(28, 15, 21, 200.00, 10.00, 2.00, 40.00, 1.00, 19.60, 2.00),
(22, 13, 1, 10.00, 120.00, 2.00, 24.00, 1.00, 11.76, 2.00),
(30, 15, 22, 50.00, 500.00, 2.00, 500.00, 4.00, 980.00, 2.00),
(27, 13, 21, 15.00, 520.00, 1.00, 78.00, 2.00, 154.44, 2.00),
(26, 13, 22, 16.00, 120.00, 2.00, 38.40, 3.00, 56.45, 2.00),
(35, 15, 21, 120.00, 10.00, 1.00, 12.00, 2.00, 23.76, 2.00),
(36, 13, 21, 10.00, 120.00, 10.00, 120.00, 1.00, 10.80, 2.00),
(37, 14, 21, 150.00, 250.00, 5.00, 1875.00, 2.00, 712.50, 2.00);
...
--
-- Dumping data for table `tbl_cash_memo_return`
--

INSERT INTO `tbl_cash_memo_return` (`cash_memo_return_id`, `customer_name`, `mobile_no`, `invoice_no`, `cash_memo_return_date`, `narration`, `sub_total`, `pay`, `place_of_supply_id`, `payment_type_id`, `financial_year_id`, `created_by`) VALUES
(2, 'Prashant Mishra', 8523654789, 1, '2024-03-02', 'For Testing Purpose', 49490.00, 49490.00, 3, 3, 2, 4),
(3, 'Anup Mishra', 8563214587, 2, '2024-03-02', 'For New ', 49572.00, 25000.00, 5, 4, 2, 4),
(4, 'Jayesh Mishra', 7874063236, 3, '2024-03-02', 'Testing For 2 tables', 7494452.00, 700000.00, 2, 4, 2, 4);
...
--
-- Dumping data for table `tbl_cash_memo_return_detail`
--

INSERT INTO `tbl_cash_memo_return_detail` (`cash_memo_return_detail_id`, `cash_memo_return_id`, `product_id`, `qty`, `rate`, `discount_per`, `discount_amt`, `gst_per`, `gst_amt`, `financial_year_id`) VALUES
(1, 4, 21, 1500.00, 5000.00, 10.00, 750000.00, 5.00, 337500.00, 2),
(2, 4, 22, 1000.00, 430.00, 9.00, 38700.00, 4.00, 15652.00, 2);
...
(5, 9, 'Income', 'Income Goodwill Recived by Last King', 45000.00, 0.00, '2024-02-14', 2),
(7, 48, 'Expense', 'Expense For Employee Tea', 0.00, 1500.00, '2024-02-14', 2),
(8, 10, 'Income', 'Income Sales To Prabhat Mishra a', 45000.00, 0.00, '2024-02-02', 2),
(9, 49, 'Expense', 'Expense holding for Adv', 0.00, 8500.00, '2024-02-10', 2),
(10, 11, 'Income', 'Income Sales Of Furniture To XYZ ', 45000.00, 0.00, '2024-02-19', 2),
(19, 13, 'CashMemo', 'CashMemo:Invoice_no:1', 0.00, 10000.00, '2024-03-01', 2),
(20, 14, 'PO', ' Kraft papers For New Worlk', 0.00, 40000.00, '2024-02-28', 2),
(21, 15, 'PO', 'Testing Purpose', 0.00, 20188.00, '2024-02-29', 2),
(30, 13, 'PO', 'Raw For Papers', 0.00, 300000.00, '2024-03-01', 2),
(31, 14, 'CashMemo', 'CashMemo:Invoice_no:2', 0.00, 2000.00, '2024-03-02', 2),
(32, 15, 'CashMemo', 'CashMemo:Invoice_no:3', 0.00, 3000.00, '2024-03-02', 2);
...
(49, 2, 2, '2024-02-10', 8500, 'holding for Adv', '', 3, 2, '2024-02-16 07:10:34');
...
INSERT INTO `tbl_item` (`item_id`, `item_name`, `quality_id`, `category_id`, `size_id`, `gsm_id`, `quantity`, `amount`, `op_stock`, `penalty`, `credit_period`, `is_active`, `item_photo`, `created_by`, `added_date`) VALUES
(1, 'Kraft Paper', 8, 92, '3,4', '1,3', 52, 4500, 456, 10, '2024-02-01', 1, 'desktop-wallpaper-the-light-of-dawn-rays-dawn-sunlight-trees-autumn-orange-leaves-forest.jpg', 4, '2024-02-12 12:16:20'),
(21, 'Gold Kraft Paper NP', 1, 92, '3,7,8', '1,8', 10, 1500, 120, 0, '0000-00-00', 0, '', 4, '2024-02-25 12:46:56'),
(22, 'SILVER OLD Paper', 3, 94, '3,7,9', '3,8,10', 12, 412300, 520, 0, '0000-00-00', 1, '', 4, '2024-02-25 12:56:20');
...
(10, 'Supplier', 23, 13, 'PO', 0, 3, 'fdsfdsfsf', 0.00, 0.00, '2024-02-28', 2, 4),
(9, 'Supplier', 27, 12, 'PO', 0, 2, 'TESTING PURPOSE ONLY.', 0.00, 351.00, '2024-02-28', 2, 4),
(8, 'Supplier', 27, 11, 'PO', 0, 1, 'TESTING PURPOSE ONLY.', 0.00, 351.00, '2024-02-28', 2, 4),
(11, 'Supplier', 1, 14, 'PO', 0, 4, ' Kraft papers For New Worlk', 0.00, 40000.00, '2024-02-28', 2, 4),
(12, 'Supplier', 28, 15, 'PO', 0, 5, 'Testing Purpose', 0.00, 20188.00, '2024-02-29', 2, 4);
...
INSERT INTO `tbl_purchase_invoice` (`purchase_invoice_id`, `party_id`, `ref_order_no`, `invoice_no`, `purchase_date`, `narration`, `sub_total`, `discount`, `pay`, `place_of_supply_id`, `payment_type_id`, `financial_year_id`, `created_by`) VALUES
(13, 23, '0', 1, '2024-02-28', 'fdsfdsfsf', 5351.50, 90.00, 0.00, 5, 1, 2, 4),
(12, 27, '0', 2, '2024-02-28', 'TESTING PURPOSE ONLY.', 5351.50, 90.00, 351.00, 5, 4, 2, 4),
(11, 27, '0', 3, '2024-02-28', 'TESTING PURPOSE ONLY.', 5351.50, 90.00, 351.00, 5, 4, 2, 4),
(14, 1, 'S002', 4, '2024-02-28', ' Kraft papers For New Worlk', 59294.00, 2800.00, 40000.00, 1, 4, 2, 4),
(15, 28, 'S003', 5, '2024-02-29', 'Testing Purpose', 20188.00, 400.00, 20188.00, 4, 1, 2, 4);
...
INSERT INTO `tbl_purchase_invoice_detail` (`purchase_invoice_detail_id`, `purchase_id`, `product_id`, `qty`, `rate`, `discount_per`, `discount_amt`, `gst_per`, `gst_amt`, `financial_year_id`) VALUES
(10, 12, 0, 20.00, 200.00, 2.00, 80.00, 10.00, 392.00, 2),
(9, 12, 1, 10.00, 100.00, 1.00, 10.00, 5.00, 49.50, 2),
(12, 13, 1, 10.00, 100.00, 1.00, 10.00, 5.00, 49.50, 2),
(13, 13, 21, 20.00, 200.00, 2.00, 80.00, 10.00, 392.00, 2),
(14, 14, 21, 10.00, 1000.00, 3.00, 300.00, 2.00, 194.00, 2),
(15, 14, 1, 20.00, 2500.00, 5.00, 2500.00, 4.00, 1900.00, 2),
(16, 15, 21, 20.00, 1000.00, 2.00, 400.00, 3.00, 588.00, 2);
...
(4, 'Prabhat Mishra', 'mprabhat846@gmail.com', 7874063236, 'prabhat', '$2y$10$1HtECWMYu906Xyuu8ES/JOVP0.kL/zRxlWB.ji.dM7f4m9kzdHS0i', 1, 0, 'wallpapersden.com_satoru-gojo-cool-jujutsu-kaisen-hd_1920x1080.jpg', '2024-01-09 09:58:10', 4),
