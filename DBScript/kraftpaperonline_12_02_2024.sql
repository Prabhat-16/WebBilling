...
--
-- Host: 127.0.0.1:3306
-- Generation Time: Feb 12, 2024 at 03:31 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertItem` (IN `INAME` VARCHAR(30), IN `QID` BIGINT(20), IN `CID` BIGINT(20), IN `SID` BIGINT(20), IN `GID` BIGINT(20), IN `QTY` BIGINT(50), IN `AMT` BIGINT(100), IN `OPSTOCK` BIGINT(50), IN `ISACTIVE` TINYINT(1), IN `ITEMPHOTO` VARCHAR(2000), IN `UID` BIGINT(20))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateItem` (IN `IID` BIGINT(20), IN `INAME` VARCHAR(30), IN `QID` BIGINT(20), IN `CID` BIGINT(20), IN `SID` BIGINT(20), IN `GID` BIGINT(20), IN `QTY` BIGINT(50), IN `AMT` BIGINT(100), IN `OPSTOCK` BIGINT(50), IN `ISACTIVE` TINYINT(1), IN `ITEMPHOTO` VARCHAR(2000), IN `UID` BIGINT(20))   BEGIN
	UPDATE tbl_item SET item_name  = INAME , quality_id = QID , category_id = CID , size_id = SID , gsm_id = GID , quantity = QTY , amount = AMT , op_stock = OPSTOCK , is_active = ISACTIVE , item_photo = ITEMPHOTO , created_by = UID WHERE item_id = IID;
END$$
...
INSERT INTO `tbl_item` (`item_id`, `item_name`, `quality_id`, `category_id`, `size_id`, `gsm_id`, `quantity`, `amount`, `op_stock`, `is_active`, `item_photo`, `created_by`, `added_date`) VALUES
(1, 'Kraft Paper', 3, 90, 3, 3, 52, 4500, 456, 0, 'desktop-wallpaper-the-light-of-dawn-rays-dawn-sunlight-trees-autumn-orange-leaves-forest.jpg', 4, '2024-02-12 12:16:20'),
(3, 'New Kraft', 15, 92, 3, 8, 42, 28000, 75, 1, 'pexels-cesar-perez-733745.jpg', 4, '2024-02-12 14:43:50');
