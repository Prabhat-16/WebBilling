...
$invoiceNumberQuery = "Select IFNull(Max(invoice_no),0)+1 as invoice_no From tbl_sales_invoice WHERE financial_year_id = (SELECT financial_year_id FROM tbl_financial_year WHERE is_default = 1)";
...
if (isset($_POST['BtnInvoice'])) 
{
    ...
    $sql_sales_invoice = "INSERT INTO tbl_sales_invoice (party_id,ref_order_no,invoice_no,due_days,sales_date,due_date,narration,net_total,discount,pay,place_of_supply_id,payment_type_id,financial_year_id,created_by) VALUE ('".$_POST['party_name']."','".$_POST['ref_order_no']."','".$invoice_no."','".$_POST['credit_period']."','".$_POST['invoice_date']."','".$_POST['due_date']."','".$_POST['narration']."','".$_POST['sub_total']."','".$_POST['discount_total']."','".$_POST['pay']."','".$_POST['state']."','".$_POST['payment_type']."','".$financial_year."','".$_SESSION['user_id']."')";
    // echo $sql_sales_invoice;
    $rs_sales_invoice = mysqli_query($con,$sql_sales_invoice);
    $last_id = mysqli_insert_id($con);
    if(!$rs_sales_invoice)
    {
        die('Not Sales Inserted.'.mysqli_error($con));
    }
    ...
    if($number > 0)
    {
        for($i=0; $i<$number; $i++)
        {
            //Insert Into Sales Invoice Detail 
            $sql_sales_invoice_detail = "INSERT INTO tbl_sales_invoice_detail(sales_invoice_id,product_id,qty,rate,discount_per,discount_amt,gst_per,gst_amt,financial_year_id) VALUE ('".$last_id."','".$_POST['product_id'][$i]."','".$_POST['quantity'][$i]."','".$_POST['rate'][$i]."','".$_POST['discount_per'][$i]."','".$_POST['discount_amt'][$i]."','".$_POST['gst_per'][$i]."','".$_POST['gst_amt'][$i]."','".$financial_year."')";
            $rs_sales_invoice_detail = mysqli_query($con,$sql_sales_invoice_detail);
            if(!$rs_sales_invoice_detail)
            {
                die('Sales Detail Not Inserted.'.mysqli_error($con));
            }
        }
    }

        //Insert Into Company Ledger
        $sql_company_ledger = "INSERT INTO tbl_company_ledger(related_id,related_obj,description,credit,debit,company_ledger_date,financial_year_id) VALUE ('".$last_id."','SO','".$description."','".$_POST['pay']."','0.00','".$_POST['invoice_date']."','".$financial_year."')";
        $rs_company_ledger = mysqli_query($con,$sql_company_ledger);
        if(!$rs_company_ledger)
        {
            die('Not Inserted In Company Ledger');
        }

        //Insert Into Party Ledger
        $sql_party_ledger = "INSERT INTO tbl_party_ledger(party_type,party_id,related_id,related_obj_name,invoice_type,invoice_no,narration,credit,debit,party_ledger_date,financial_year_id,created_by) VALUE ('Customer','".$_POST['party_name']."','".$last_id."','SO','0','".$invoice_no."','".$description."','".$_POST['pay']."','0.00','".$_POST['invoice_date']."','".$financial_year."','".$_SESSION['user_id']."')";
        mysqli_query($con,$sql_party_ledger);
        if(!$rs_company_ledger)
        {
            die('Not Inserted in Party Ledger');
        }
