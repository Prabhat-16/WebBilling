...
$sql_view = "SELECT tpi.*, tp.party_name, tp.address, tp.contact_no, tp.email, tpy.payment_type, ts.state_name
            FROM tbl_purchase_invoice tpi
            LEFT JOIN tbl_party tp ON tpi.party_id = tp.party_id
            LEFT JOIN tbl_payment_type tpy ON tpi.payment_type_id = tpy.payment_type_id
            LEFT JOIN tbl_state ts ON tpi.place_of_supply_id = ts.state_id WHERE purchase_invoice_id = '" . $_GET['id'] . "'  ";
...
$sql_detail_view = "SELECT tpid.*, ti.item_name 
                    FROM tbl_purchase_invoice_detail tpid
                    LEFT JOIN tbl_item ti ON tpid.product_id = ti.item_id
                    WHERE purchase_invoice_id = '" . $_GET['id'] . "'";
...
// Add Title
$pdf->SetFont('Arial', 'B', 20);
$pdf->Cell(189, 10, 'Purchase Invoice', 0, 1, 'C');
$pdf->Ln(5);
...
$pdf->Cell(30, 5, date('d F Y', strtotime($row_view['purchase_date'])), 0, 0, 'R');
$pdf->Ln(5);
...
// Save PDF to a file
$pdfPath = './fpdf/purchase_' . $_GET['id'] . '.pdf';
                        ...
                        <div class="invoice-header">
                            ...
                            <div class="row justify-content-between">
                                ...
                                <div class="col-xl-5 col-lg-5 col-md-5 col-sm-5 col-12">
                                   <strong style="
                                    margin: -62%;
                                    font-size: 34px; font-family: Arial, sans-serif;">Purchase Invoice</strong>
                                </div>
                            </div>
                            <!-- Row end -->
                        </div>
                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                <div class="invoice-details">
                                    ...
                                    <address class="m-0">
                                        ...
                                        <div> Purchase  Date :-
                                            <?php
                                            $purchase_date_timestamp = strtotime($row_view['purchase_date']);
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        //    $paise = ($decimal > 0) ? "." . ($words[$decimal / 10] . " " . $words[$decimal % 10]) . ' Paise' : '';
                                           return ($Rupees ? $Rupees . 'Rupees ' : '');
                                            ...
                                            $total_gtotal = 0;
                                            $total_amt = 0;
                                            $sql_detail_view1 = "SELECT tpid.*, ti.item_name 
                                                                FROM tbl_purchase_invoice_detail tpid
                                                                LEFT JOIN tbl_item ti ON tpid.product_id = ti.item_id
                                                                WHERE purchase_invoice_id = '" . $_GET['id'] . "'";
