...
$sql_view = "SELECT tsi.*, tp.party_name, tp.address, tp.contact_no, tp.email, tpy.payment_type, ts.state_name
            FROM tbl_sales_invoice tsi
            LEFT JOIN tbl_party tp ON tsi.party_id = tp.party_id
            LEFT JOIN tbl_payment_type tpy ON tsi.payment_type_id = tpy.payment_type_id
            LEFT JOIN tbl_state ts ON tsi.place_of_supply_id = ts.state_id WHERE sales_invoice_id = '" . $_GET['id'] . "'  ";
...
$sql_detail_view = "SELECT tsid.*, ti.item_name 
                    FROM tbl_sales_invoice_detail tsid
                    LEFT JOIN tbl_item ti ON tsid.product_id = ti.item_id
                    WHERE sales_invoice_id = '" . $_GET['id'] . "'";
...
function getIndianCurrency1(float $number)
    ...
    $number_show =  $row_view['net_total'];
...
// Add Title
$pdf->SetFont('Arial', 'B', 20);
$pdf->Cell(189, 10, 'Sales Invoice', 0, 1, 'C');
$pdf->Ln(5);
...
$pdf->Cell(160, 5, $row_view['party_name'], 1, 1);
$pdf->Cell(30, 5, 'Contact No:', 1, 0);
$pdf->Cell(160, 5, $row_view['contact_no'], 1, 1);
$pdf->Cell(30, 5, 'Email:', 1, 0);
$pdf->Cell(160, 5, $row_view['email'], 1, 1);
$pdf->Cell(30, 5, 'Address:', 1, 0);
$pdf->MultiCell(160, 5, $row_view['address'], 1, 'L');
...
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(60, 5, 'Purchase Date:', 1, 0, 'R');
$pdf->Cell(30, 5, date('d F Y', strtotime($row_view['sales_date'])), 0, 0, 'R');
...
$pdf->Ln(20); // Add spacing for better readability
// Table Header
...
$pdf->Cell(54, 10, $row_view['net_total'], 1, 0, 'R');
...
// Save PDF to a file
$pdfPath = './fpdf/sales_' . $_GET['id'] . '.pdf';
                        ...
                        <div class="invoice-header">
                            ...
                            <div class="row justify-content-between">
                                ...
                                <div class="col-xl-5 col-lg-5 col-md-5 col-sm-5 col-12">
                                   <strong style="
                                    margin: -62%;
                                    font-size: 34px; font-family: Arial, sans-serif;">Sales Invoice</strong>
                                </div>
                            </div>
                            <!-- Row end -->
                        </div>
                        ...
                        <div class="row gutters">
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                <div class="invoice-details">
                                    <address class="m-0">
                                       <div>Party Name :- <?php echo $row_view['party_name']; ?></div>
                                       <div>Contact No :- <?php echo $row_view['contact_no']; ?></div>
                                    ...
                                    <address class="m-0">
                                        ...
                                        <div> Purchase  Date :-
                                            <?php
                                            $purchase_date_timestamp = strtotime($row_view['sales_date']);
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                          ...
                                          //    $paise = ($decimal > 0) ? "." . ($words[$decimal / 10] . " " . $words[$decimal % 10]) . ' Paise' : '';
                                            ...
                                            $total_gtotal = 0;
                                            $total_amt = 0;
                                            $sql_detail_view1 = "SELECT tsid.*, ti.item_name 
                                                                FROM tbl_sales_invoice_detail tsid
                                                                LEFT JOIN tbl_item ti ON tsid.product_id = ti.item_id
                                                                WHERE sales_invoice_id = '" . $_GET['id'] . "'";
                                            ...
                                            <td colspan="5"><h4 class="mt-2 text-danger"><strong>In Words:</strong></h4><h5 class="mt-2 text-danger"><?php echo $paidAmountInWords; ?> only</h5></td>
                                                ...
                                                <td>
                                                    ...
                                                    <h5 class="mt-2 text-danger"><?php echo $row_view['net_total']; ?></h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
